...
design variables (currently midWidth, midDist and inner fillet)
...
def modifyDimensions(modelData, width=(dim[3]-dim[0]),height=(dim[4]-dim[1]),depth=0):
	"""Modify substructure dimensions with function taking in dimensions defined by dim list by default
	
	Arguments:
		modelData: Dictionary with model data
	...
	"""
	p = mdb.models[modelData['modelName']].parts[modelData['partName']]
	s = p.features['Shell planar-1'].sketch
	mdb.models[modelData['modelName']].ConstrainedSketch(name='__edit__', objectToCopy=s)
	s1 = mdb.models[modelData['modelName']].sketches['__edit__']
	g, v, d, c = s1.geometry, s1.vertices, s1.dimensions, s1.constraints
	s1.setPrimaryObject(option=SUPERIMPOSE)
	p.projectReferencesOntoSketch(sketch=s1, 
		upToFeature=p.features['Shell planar-1'], filter=COPLANAR_EDGES)
		
	s=mdb.models[modelData['modelName']].sketches['__edit__']
...
def modifyArcs(modelData,cornerSize,midWidth,midDist,fillet):
	...
	Arguments:
		...
		midWidth & midDist: Percentages used to define thicknesses of cross members in quadrants 1/3 or 2/4
	...
	s.parameters['MidWidth'].setValues(expression=str(midWidth)+'*SubWidth')
	s.parameters['MidDist'].setValues(expression='(0.5+('+str(midDist)+'))*SubWidth') # (0.5+(midDist))*SubWidth
	
	p = mdb.models[modelData['modelName']].parts[]
...
def runModel(modelData,i):
	...
	# Create Job
	jobName=modelData['partName']+'-'+str(i)
	job= mdb.Job(name=jobName, model=modelData['modelName'], description='', type=ANALYSIS, 
...
def takeScreenshot(modelData,innerR,thickness):
	'''
	...
	p = mdb.models['Model-1'].parts[modelData['partName']]
...
def createAnalyticalSurface(modelData,dim):
	'''
	...
	minX = dim[0]
	maxX = dim[3]
	s1 = mdb.models[modelData['modelName']].ConstrainedSketch(name='__profile__', 
		sheetSize=200.0)
	g, v, d, c = s1.geometry, s1.vertices, s1.dimensions, s1.constraints
	s1.setPrimaryObject(option=STANDALONE)
	s1.Line(point1=(minX*1.5, 0.0), point2=(maxX*1.5, 0.0))
	p = mdb.models[modelData['modelName']].Part(name='Surface', dimensionality=THREE_D, 
		type=ANALYTIC_RIGID_SURFACE)
	p = mdb.models[modelData['modelName']].parts['Surface']
...
def instanceAnalyticalSurface(modelData,dim):
	'''
	...
	yMax = dim[4]
	# Instance Part 
	a1 = mdb.models[modelData['modelName']].rootAssembly
	p = mdb.models[modelData['modelName']].parts['Surface']
...
def defineRigidBody(modelData,dim):
	'''
	...
	a = mdb.models[modelData['modelName']].rootAssembly
	...
	mdb.models[modelData['modelName']].RigidBody(name='RigidBody', refPointRegion=region1, 
		surfaceRegion=region5)
	
	return 


def tieBody_and_Part(modelData,dim):
	'''
	...
	a = mdb.models[modelData['modelName']].rootAssembly
	e1 = a.instances[modelData['partName']+'-1'].edges
	edges1 = e1.getByBoundingBox(1.5*minX,0.95*maxY, -1.0,1.5*maxX,1.05*maxY,1.0)
	region2=a.Set(edges=edges1, name='Top_Edge')
	mdb.models[modelData['modelName']].Tie(name='Tie', master=region1, slave=region2, 
...
def modifyStep(modelData):
	...
	try:
		del mdb.models[modelData['modelName']].steps['Step-1']
	except:
		print('Step-1 Does not exist')
		
	#Create a static, general step 
	mdb.models[modelData['modelName']].StaticStep(name='Step-1', previous='Initial')
	return 


def encastreBottom(modelData,dim):
	...
	minY = dim[1]
	a1 = mdb.models[modelData['modelName']].rootAssembly
	e1 = a1.instances[modelData['partName']+'-1'].edges
	edges1 = e1.getByBoundingBox(1.5*minX,1.05*minY, -1.0,1.5*maxX,0.95*minY,1.0)
	region = a1.Set(edges=edges1, name='Set-3')
	mdb.models[modelData['modelName']].EncastreBC(name='FIX_BOTTOM', createStepName='Initial', 
		region=region, localCsys=None)
	
	return 


def xDisplacement(modelData,disp,subWidth):
	...
	mdb.models[modelData['modelName']].DisplacementBC(name='MOVE_RP', createStepName='Step-1',
...
def createJob(modelData,jobName):
	''' Creates a job to correspond to the x-displacement load case'''
	mdb.Job(name=jobName, model=modelData['modelName'], description='', type=ANALYSIS, 
		atTime=None, waitMinutes=0, waitHours=0, queue=None, memory=90, 
		memoryUnits=PERCENTAGE, getMemoryFromAnalysis=True, 
		explicitPrecision=SINGLE, nodalOutputPrecision=SINGLE, echoPrint=OFF, 
		modelPrint=OFF, contactPrint=OFF, historyPrint=OFF, userSubroutine='', 
...
def analysisStep(modelData,dim,theta,thickness):
	'''
	...
	mdb.models[modelData['modelName']].StaticStep(name='Analysis', previous='Initial')
	...
	mdb.models[modelData['modelName']].EncastreBC(name='Encastre', createStepName='Initial', 
		region=region, localCsys=None)
...
midWidthset = np.arange(0.05,0.21,0.05)	# Bounds unknown
midDistset = np.arange(-0.27,0.28,0.18)	# +/- (0.5-midWidth)
filletSet = np.arange(0.04,0.18,0.045)				# Bounds unknown
materials = np.array(['Titanium Alpha-Beta'])#'Aluminum 2219','Kovar Steel','Titanium Alpha-Beta'])

nSubs = getSetSize(midWidthset, midDistset, filletSet, materials)
...
counter = 1
fileDataName = 'VerticalArcsDataPoints.csv'
...
with open(fileDataName, 'wb') as csvfile:
		...
		'MidWidth  Min:', midWidthset[0]*width, ' Max:',midWidthset[-1]*width, ' Step:',(midWidthset[-1]*width-midWidthset[0]*width)/(midWidthset.size-1),
		'MidDist  Min:', midDistset[0]*width, ' Max:',midDistset[-1]*width, ' Step:',(midDistset[-1]*width-midDistset[0]*width)/(midDistset.size-1),
		...
		'Substructure','ID','Material','K_xy','K_y','K_theta','mass','maxMises1','maxMises2','maxMises3','MidWidth','MidDist','Fillet')
		csvfile.write(headers)

for material in materials:				
	for midWidth in midWidthset:
		for midDist in midDistset:
			for fillet in filletSet:
				Mdb()
				openMdb(model)			
				
				changeMaterial(sectionSet[0],material)
				dim = modifyDimensions(modelData)
				modifyArcs(modelData=modelData,cornerSize=cornerSize,midWidth=midWidth,midDist=midDist,fillet=fillet)

				# force = analysisStep(model,dim,theta=theta,thickness = thickness )
				mesh(modelData=modelData, globalSeed=0.03*width) #Need to change maybe
				#Run a substructure generation step in parallel 
				subJobName = substructureModel(modelData,i=counter,matricesFile = 'MATRICES')
				time.sleep(45)  # Used to troubleshoot dual processes running at different speeds

				#Change after this point
				
				createAnalyticalSurface(modelData=modelData,dim=dim)
				instanceAnalyticalSurface(modelData,dim)
				defineRigidBody(modelData,dim)
				
				tieBody_and_Part(modelData,dim)
				modifyStep(modelData)
				encastreBottom(modelData,dim)
				postProcessingSets(modelData,dim)
				
				# Stiffness k_x
				xDisplacement(modelData,5e-4,width)
				xDisplacementJob = createJob(modelData=modelData,jobName='X_DISP')
				...
				# Stiffness k_y
				mdb.models[modelData['modelName']].boundaryConditions['MOVE_RP'].setValues(u1=0.0, u2=(5e-4)*(dim[4]-dim[1]))
				yDisplacementJob = createJob(modelData=modelData,jobName='Y_DISP')
				...
				# Stiffness k_theta
				mdb.models[modelData['modelName']].boundaryConditions['MOVE_RP'].setValues(u2=0.0, ur3=0.01) # In degress at the moment, need to check if it needs to be in radians
				thetaRotationJob = createJob(modelData=modelData,jobName='THETA_ROT')
				...
				# Query mass
				mass = mdb.models[modelData['modelName']].parts[modelData['partName']].getMassProperties()['mass']
				...
				with open(fileDataName, 'ab') as csvfile:
					...
					'twoArcs',counter,material,K_xy,K_y,K_theta,mass,maxMises1,maxMises2,maxMises3,midWidth,midDist,fillet)
					csvfile.write(txt)
				counter += 1
